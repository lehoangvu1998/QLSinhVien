...
public class DAL_GV {
    ...
    public ArrayList<DTO_GV> GetListGiangVien() {
        ArrayList<DTO_GV> list = new ArrayList<>();
        String sql = "SELECT us.MAUSERS, us.HOTEN, us.SDT, us.NGAYSINH, us.EMAIL, us.DIACHI, us.PASS, us.HOCVAN, us.NGAYVAOTRUONG, kh.TENKHOA, tn.TENTHANNHAN, tn.SDTTHANNHAN, tn.QUANHE FROM USERS us, KHOA kh, THANNHAN tn WHERE tn.MAUSERS = us.MAUSERS and us.ROLE = 1";
        try {
    ...
    public int insertGiangVien(int msnv, String hoten, String dienthoai, Date ngaysinh, String email, String diachi, String matkhau, String hocvan, int makhoa,String malop, int maphongban, int role, Date ngayvaotruong) {
        int result = 0;
        String query_giangvien = "insert into USERS (MAUSERS, HOTEN, SDT, NGAYSINH, EMAIL, DIACHI, PASS, HOCVAN, MAKHOA, MALOP, MAPHONGBAN, ROLE, NGAYVAOTRUONG) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try {
            db = new DatabaseConnection();
            con = db.getConnection();
            ps = con.prepareStatement(query_giangvien);
            ps.setInt(1, msnv);
